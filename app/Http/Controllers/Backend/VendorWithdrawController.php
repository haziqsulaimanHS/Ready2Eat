...
class VendorWithdrawController extends Controller
{
    ...
    {
        $totalEarningsQuery = OrderProduct::where('vendor_id', auth()->user()->id)
            ->whereHas('order', function($query){
                $query->where('payment_status', 1)->where('order_status', 'delivered');
            });

        // Check if there are any orders
//        if ($totalEarningsQuery->exists()) {
            $totalEarnings = $totalEarningsQuery->sum(DB::raw('unit_price * qty + variant_total'));
...
//        } else {
//            // Set to 0 if there are no orders
//            $totalEarnings = 0;
//            $totalWithdraw = 0;
//            $currentBalance = 0;
//            $pendingAmount = 0;
