            ...
            <div class="col-xl-12">
                <div class="relative_contect d-flex">
                    ...
                    <ul class="wsus_menu_cat_item show_home toggle_menu">
                        <!-- Add the "All Products" link at the beginning of the list -->
                        @if (count($categories) > 0)
                            <li><a href="{{ route('products.index', array_merge(request()->query(), ['subcategory' => null])) }}"><i class="fas fa-list"></i> All Products</a></li>
                        @endif

                        @foreach ($categories as $category)
                            <li>
                                <a class="{{ count($category->subCategories) > 0 ? 'wsus__droap_arrow' : '' }}" href="{{ route('products.index', array_merge(request()->query(), ['category' => $category->slug])) }}">
                                ...
                                @if(count($category->subCategories) > 0)
                                    <ul class="wsus_menu_cat_droapdown">
                                        @foreach ($category->subCategories as $subCategory)
                                            <li>
                                                <a href="{{ route('products.index', array_merge(request()->query(), ['subcategory' => $subCategory->slug])) }}">
                                                    {{ $subCategory->name }}
                                                    <i class="{{ count($subCategory->childCategories) > 0 ? 'fas fa-angle-right' : '' }}"></i>
                                                </a>
                                                @if(count($subCategory->childCategories) > 0)
                                                    <ul class="wsus__sub_category">
                                                        @foreach ($subCategory->childCategories as $childCategory)
                                                            <li>
                                                                <a href="{{ route('products.index', array_merge(request()->query(), ['childcategory' => $childCategory->slug])) }}">
                                                                    {{ $childCategory->name }}
                                                                </a>
                                                            </li>
                                                        @endforeach
